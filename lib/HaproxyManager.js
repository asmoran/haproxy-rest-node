...
handlebars.registerHelper('backendHelper', function (backend) {
  ...
  if (members) {
    ...
    members.forEach(function (member) {
	  ...
	  var options = (member.opts) ? member.opts : "";
      output.push(util.format("server %s %s:%s check inter %s %s", member.name, member.host, member.port, interval, options));
    });
  }
